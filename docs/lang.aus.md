...
[2594:14] (extern: com.aussom.stdlib.ASecurityManager) **extends: object** 

The SecurityManager class provides an object that you can
instantiate and provide to the Aussom engine to use.
What you get from new SecurityManager() is a policy value,
not the policy of the engine you are running in. Writing to
it does not change what the running program may do; the
static secman class is the one that reads the engine's own
policy.
...
- **SecurityManager** ()
	...
	> Creates a new security manager object. This function requires Security Manager permission securitymanager.instantiate.

	- **@r** `A` new SecurityManager object.
